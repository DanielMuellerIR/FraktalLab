...
const unimplementedEffects = new Set<number>();

class FallbackChannel {
  worklet: FallbackModPlayerProcessor;
  channelIndex: number;
  instrument: any | null = null;
  playing = false;
  period = 0;
  currentPeriod = 0;
  portamentoSpeed = 0;
  ...
  // Effects temporary states
  volumeSlide = 0;
  portamento = false;
  vibrato = false;
  retrigger: any = false;
  delayNote: any = false;
  setInstrument: any = false;
  setVolume: any = false;
  setPeriod: any = false;
  setCurrentPeriod = false;
  setSampleIndex: any = false;

  constructor(worklet: FallbackModPlayerProcessor, index: number) {
  ...
  effect(note: Note) {
    ...
    if (!note.hasEffect) return;

    const { effectId, effectData, effectHigh, effectLow } = note;
...
export class FallbackModPlayerProcessor {
  mod: Mod | null = null;
  sampleRate = 44100;
  channels: FallbackChannel[];
  patternBreak: any = false;
  publishRow = false;
  publishStop = false;
  publishNote = false;
  ...
  // Communication callback simulating AudioWorkletPort
  onportmessage: ((event: { data: any }) => void) | null = null;

  constructor() {
    this.channels = [
      new FallbackChannel(this, 1),
      new FallbackChannel(this, 2),
      new FallbackChannel(this, 3),
      new FallbackChannel(this, 4),
    ];
  }

  sendPortMessage(data: any) {
  ...
  play(mod: Mod, sampleRate: number) {
    this.mod = mod;
  ...
  setBpm(bpm: number) {
    this.bpm = bpm;
    this.outputsPerTick = ((this.sampleRate * 60) / this.bpm / 4 / 6) | 0;
    if (bpm === 0 && this.publishStop) {
      this.sendPortMessage({ type: 'stop' });
    }
  }
  ...
  process(_inputs: any, outputs: Float32Array[][]) {
    try {
      ...
      console.error("Fallback processor error in process:", e);
    }
    return true;
  }
}
